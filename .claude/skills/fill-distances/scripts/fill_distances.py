...
def process_route(route_text, distances, start_point):
    ...
    返回: [(站点名, 距离, 查询键, 是否找到), ...]
    """
    if route_text is None or route_text == '':
        return []
...
def get_default_cache_path(region):
    """
    获取默认的缓存文件路径
    """
    if region == 'hefei':
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python fill_distances.py <excel_file> [distances_json] [region]")
        print()
        print("Parameters:")
        print("  excel_file      : 对账单Excel文件路径")
        print("  distances_json  : 可复用距离JSON文件路径（可选，默认使用区域缓存）")
        print("  region          : 区域 (hefei/jiangxi，可选，默认自动检测)")
        print()
        print("Examples:")
        print("  # 使用默认缓存，自动检测区域")
        print("  python fill_distances.py data/hefei/summary/2026/惠宜选合肥仓1月份对账单0119.xlsx")
        print()
        print("  # 指定距离JSON文件")
        print("  python fill_distances.py data/hefei/summary/2026/惠宜选合肥仓1月份对账单0119.xlsx \\")
        print("                           data/hefei/cache/reusable_distances.json")
        print()
        print("  # 指定区域")
        print("  python fill_distances.py data/jiangxi/summary/2026/江西对账单0113.xlsx \\")
        print("                           data/jiangxi/cache/reusable_distances.json \\")
        print("                           jiangxi")
        sys.exit(1)
    ...
    if region not in START_POINTS:
        print(f"Error: 无法识别区域。请指定 region 参数 (hefei/jiangxi)")
        print(f"检测到的区域: {region}")
        sys.exit(1)

    start_point = START_POINTS[region]

    # 确定距离JSON文件路径
    if len(sys.argv) > 2:
        json_path = sys.argv[2]
    else:
        json_path = get_default_cache_path(region)
        if json_path is None:
            print(f"Error: 无法确定默认缓存路径，请手动指定")
            sys.exit(1)

    if not os.path.exists(json_path):
        print(f"Error: 距离JSON文件不存在 - {json_path}")
        sys.exit(1)
    ...
    ws = wb.active
    print(f"Excel尺寸: {ws.max_row} 行 x {ws.max_column} 列")
