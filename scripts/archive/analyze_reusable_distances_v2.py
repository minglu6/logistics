...
def parse_shop_and_distance(shop_line):
    ...
    格式示例：
    - "共橙超市-六安舒城店-98km" -> ("共橙超市-六安舒城店", 98.0)
    - "惠宜选超市（六安霍山店）-73km" -> ("惠宜选超市（六安霍山店）", 73.0)
    - "共橙超市-六安舒城店" -> ("共橙超市-六安舒城店", None)
    """
    shop_line = shop_line.strip()

    # 匹配最后一个"-数字km"或"-数字.数字km"
    pattern = r'^(.+?)-(\d+(?:\.\d+)?)km$'
...
def extract_routes_from_excel(file_path, has_distance_in_name=False):
    ...
    Args:
        file_path: Excel文件路径
        has_distance_in_name: 店名中是否包含距离信息（12月份为True，1月份为False）
...
def analyze_reusable_distances(jan_file, dec_file):
    ...
    # 读取1月份数据（店名中没有距离）
    print("\n读取1月份对账单...")
    jan_routes = extract_routes_from_excel(jan_file, has_distance_in_name=False)
    ...
    for jan_seg in jan_segments:
        ...
        if jan_seg['distance'] is None and key in dec_distance_map:
            ...
            reusable_segments.append({
                ...
                'dec_all_distances': dec_distances
            })
            reusable_count += 1
    ...
    if reusable_segments:
        ...
        print(f"\n总共 {len(segment_groups)} 个不同的路段可以复用")

        for segment_key, occurrences in sorted(segment_groups.items()):
            dec_distances = occurrences[0]['dec_all_distances']
            avg_dist = occurrences[0]['dec_distance']
            dec_count = occurrences[0]['dec_occurrences']
            jan_count = len(occurrences)

            # 如果12月份该路段距离有变化，显示所有值
            if len(set(dec_distances)) > 1:
                dist_str = f"{avg_dist:.2f} km (平均，12月份值: {dec_distances})"
            else:
                dist_str = f"{avg_dist:.2f} km"

            print(f"\n路段: {segment_key}")
            print(f"  可复用距离: {dist_str}")
            print(f"  12月份出现: {dec_count} 次")
            print(f"  1月份出现: {jan_count} 次")
            print(f"  1月份车次: ", end="")
            vehicles = sorted(set([int(occ['jan_vehicle']) for occ in occurrences]))
            print(", ".join(map(str, vehicles)))
    ...
    if missing_segments:
        ...
        print("\n前30个需要填充的路段:")
        for i, (segment_key, occurrences) in enumerate(list(segment_groups.items())[:30], 1):
            jan_count = len(occurrences)
            vehicles = sorted(set([int(occ['jan_vehicle']) for occ in occurrences]))
            vehicle_str = ", ".join(map(str, vehicles[:5]))
            if len(vehicles) > 5:
                vehicle_str += f", ... (共{len(vehicles)}辆车)"
            print(f"{i}. {segment_key}")
            print(f"   1月份出现 {jan_count} 次，车次: {vehicle_str}")
    ...
    for segment_key, occurrences in segment_groups.items():
        if occurrences and 'dec_distance' in occurrences[0]:
            reusable_json[segment_key] = round(occurrences[0]['dec_distance'], 2)
